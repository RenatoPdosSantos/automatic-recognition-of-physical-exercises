...
More importantly, the accuracy obtained in the *myTesting* subset (`r unname(confMx[3][[1]][1])`) is comparable with the one obtained with the *myTraining* dataset (`r max(rf_model$results$Accuracy)`), what suggests that an overfitting is not happening and, therefore, that the out-of-the-sample error is not big, being the in-sample error even smaller.

With such good results, we proceed to generate the file with the predictions to submit for the assignment.
```{r fileGeneration}
